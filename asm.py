#!/usr/bin/env python
#
# $Id: asm.py 486 2014-10-04 14:38:45Z coelho $
#
...
command = re.compile(r'BGZ|BEZ|PUSH|LOAD|STORE|SWAP|ADD|SUB|MUL|DIV|AND|OR|NOT|STOP|GOTO|IN|OUT')
commandArg = re.compile(r'BGZ|BEZ|PUSH')

Arg = re.compile(r'\d+[_@%a-zA-Z]\w*|')
        ...
        if label_re.match(l):
            ...
            if commandArg.match(cmd):
                if len(tokens) < 2:
                    ...
                    err('Bad argument for %s at line %d: %s' % (cmd, ln, tokens[1]))
            elif len(tokens)>1:
                err('Command %s does not except arguments (%s) at line %d' % \
                    (cmd, tokens[1], ln))
            line += len(l.split(None))
        else:
            err('Unknown command "%s" at line %d' % (token[0], ln))
...
for l in lines:
    ...
    if push.match(l):
        ...
        try:
            ...
            err('Undefined key %s at line %d' % (tokens[1], ln))
    elif something.match(l) and \
         not (var_re.match(l) or label_re.match(l) or comment_re.match(l)):
        print(l)
